...
| `run-cs-body-parser-tests.sh` | validation-routine body parsing per language: the C# `{`/`}` parser (simple, nested, consecutive, brace-on-signature-line, expression-bodied), a golden byte-comparison proving the Pascal `begin`/`end;` path is untouched, and a mixed `.pas` + `.cs` run (needs `jq`) |
| `run-bf-status-tests.sh` | the brownfield phase dashboard **and** the `--next` guidance block: the no-write invariant asserted against the whole project tree for both modes, absence never inferred from a sibling document, latest-replay-verdict **per target** (an older `FAIL` on one change must not vanish behind a newer `PASS` on another — the regression this suite exists for), stub taint never a false positive, multi-line clarification answers counted as answered, and graceful degradation with no `jq`. For the guidance: phase-by-phase progression, human **actions** distinguished from **commands** (and rendered first), `--resolve` ranked by what actually refuses without `decisions.md`, a post-bootstrap recommendation of `/specclaw:propose` rather than a replay of an application nobody has built, a `--not-applicable` manifest never read as a standing foundation, an outstanding replay `FAIL` stated without any line implying the phase advanced, no guidance at all from a refused run, and the structural PD-01 proof that all ten lifecycle skills reach the ordering through this script while `bf-status`/`bf-quality`/`bf-e2e` carry none. The no-write and per-target checks are mutation-verified; the CR-in-output check is a property assertion the suite documents as not currently mutation-detectable |
| `run-quality-tests.sh` | the code-quality collector: per-language coverage and `NOT-MEASURED` reason precedence (`language_unsupported` beats `tool_missing`), a missing tool degrading only its own metrics, threshold-band boundaries and the worst-status rollup, `QI-###` permanence (identical ids on an unchanged re-run, a cleared hotspot marked `resolved` rather than removed, reopening under the original id, and that a `--target` run registers nothing), compare classification incl. the unmeasured-legacy-is-not-an-improvement case, the gate's exit codes, the module join's `MOD-UNASSIGNED` and ambiguity handling, and that `bf-rebuild-plan` renders byte-identically with no `quality.json` (needs `jq`; metric tools are stubbed on PATH) |
