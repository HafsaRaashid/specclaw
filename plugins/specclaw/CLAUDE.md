...
| `specclaw-bf-status` | Per-**phase** brownfield dashboard to stdout: one row per `bf-*` phase, the open items holding each back, and the next command. Writes nothing — no file, no cache, no archive entry. jq optional. Complements `specclaw-bf-rebuild-collect module-status`, which is the per-**module** view and *is* a written artifact |
...
| `run-bf-status-tests.sh` | the brownfield phase dashboard: the no-write invariant asserted against the file tree, absence never inferred from a sibling document, latest-replay-verdict **per target** (an older `FAIL` on one change must not vanish behind a newer `PASS` on another — the regression this suite exists for), stub taint never a false positive, multi-line clarification answers counted as answered, and graceful degradation with no `jq`. The no-write and per-target checks are mutation-verified; the CR-in-output check is a property assertion the suite documents as not currently mutation-detectable |
